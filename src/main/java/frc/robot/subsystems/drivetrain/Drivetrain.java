...
public class Drivetrain extends SubsystemBase {
  ...
  public Rotation2d getTilt() {
    if (gyroInputs.connected) {
      return Rotation2d.fromDegrees(gyroInputs.tiltDeg);
    } else {
  ...
  public void periodic() {
    ...
    if (this.hasLimelight()) {
      double[] pose3d = limelight.getBotPoseFs();
      Pose2d limelightPose2d =
          new Pose2d(
              pose3d[0] + Limelight.fieldLong / 2,
              pose3d[1] + Limelight.fieldShort / 2,
              Rotation2d.fromDegrees(pose3d[5]));
      double tl = pose3d[6];
  ...
  public void setXStance() {
    ...
    states[3].angle = new Rotation2d(3.0 / 2.0 * Math.PI - Math.atan(trackwidthMeters / wheelbaseMeters));
    for (SwerveModule swerveModule : swerveModules) {
  ...
  /** Calibrates the angle motors with the mag encoder offsets. */
  public void calibrateDrive() {
    for (int i = 0; i < swerveModules.length; i++) {
      swerveModules[i].calibrateAngleMotor();
    }
  }
